...
client.settings4.ensure(guildid, {
    channel: "",
    channelname: "{user}' VC",
    guild: guildid,
});
client.settings5.ensure(guildid, {
    channel: "",
    channelname: "{user}' Chat",
    guild: guildid,
});
}
...
client.settings4.set(guildid, {
    channel: "",
    channelname: "{user}' VC",
    guild: guildid,
});
client.settings5.set(guildid, {
    channel: "",
    channelname: "{user}' Chat",
    guild: guildid,
});
...
try {
client.user.setActivity(`${config.prefix}help | ${config.prefix}setup | ${totalGuilds} Guilds`, {
    type: "WATCHING",
    shardID: shard
});
} catch (e) {
    client.user.setActivity(`${config.prefix}help | ${config.prefix}setup | ${client.guilds.cache.size} Guilds`, {
        type: "WATCHING",
        shardID: 0
    });
}
...
else if (type == 4) chname = client.settings3.get(user.member.guild.id, "channelname")
else if (type == 5) chname = client.settings3.get(user.member.guild.id, "channelname")
