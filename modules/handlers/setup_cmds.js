    ...
    if (cmd === "setup") {
        ...
        if (channel) {
          message.reply(new Discord.MessageEmbed()
            .setTitle("<:ChauvinAutoChat:915800939737280523> Setup Complete for Join to Create")
            .setColor(ee.color)
        ...
        message.reply(new Discord.MessageEmbed()
          .setTitle("<:ChauvinAutoChat:915800939737280523> Successfully changed the Channelname for the temp. Channels")
          .setColor(ee.color)
        ...
        if (channel) {
          message.reply(new Discord.MessageEmbed()
            .setTitle("<:ChauvinAutoChat:915800939737280523> Setup 2 Complete for Join to Create")
            .setColor(ee.color)
            .setDescription(`Bound to Channel: \`${channel.name}\`\nPlease rejoin!`)
            .setFooter(ee.footertext, ee.footericon)
          );
        ...
        if (channel) {
          message.reply(new Discord.MessageEmbed()
            .setTitle("<:ChauvinAutoChat:915800939737280523> Setup 3 Complete for Join to Create")
            .setColor(ee.color)
            .setDescription(`Bound to Channel: \`${channel.name}\`\nPlease rejoin!`)
            .setFooter(ee.footertext, ee.footericon)
          );
        ...
        message.reply(new Discord.MessageEmbed()
          .setTitle("<:ChauvinAutoChat:915800939737280523> Successfully changed the Channelname for the temp. Channels")
          .setColor(ee.color)
          .setDescription(`New Channelname: \`${client.settings4.get(message.guild.id, "channelname")}\`\n\nWhat it could look like: \`${client.settings4.get(message.guild.id, "channelname").replace("{user}", message.author.username)}\``)
          .setFooter(ee.footertext, ee.footericon)
        );
        return;
      } else if (cmd === "setup5") {
        ...
        if (channel) {
          ...
          client.settings4.set(message.guild.id, channel.id, `channel`);
        } else {
          message.guild.channels.create("Join to Create", {
            type: 'voice',
            bitrate: 8000,
        ...
        return;
      } else if (cmd === "setup5name") {

        if (!message.member.hasPermission("ADMINISTRATOR")) return message.reply(new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.footertext, ee.footericon).setTitle(":x: You don't have enough Permissions!"));
        if (!args[0]) return message.reply(new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.footertext, ee.footericon).setTitle(":x: You didn't add a Channelname").setDescription(`Useage: \`${prefix}setupname [new Channel Name]\` | Note: {user} will be replaced with username*`));
        if (args[0].length > 32) return message.reply(new Discord.MessageEmbed().setColor(ee.color).setFooter(ee.footertext, ee.footericon).setTitle(":x: Your provided Channel Name is too Long").setDescription(`The maximum length for a Channel name is \`32\` Letters`));
        client.settings5.set(message.guild.id, args.join(" "), "channelname");
        message.reply(new Discord.MessageEmbed()
          .setTitle("<:ChauvinAutoChat:915800939737280523> Successfully changed the Channelname for the temp. Channels")
          .setColor(ee.color)
          .setDescription(`New Channelname: \`${client.settings5.get(message.guild.id, "channelname")}\`\n\nWhat it could look like: \`${client.settings5.get(message.guild.id, "channelname").replace("{user}", message.author.username)}\``)
          .setFooter(ee.footertext, ee.footericon)
        );
        return;
      } 
